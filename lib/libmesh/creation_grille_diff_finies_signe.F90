SUBROUTINE creation_grille_diff_finies_signe
...
!write(6,*) nombre_noeuds_DF
  ...
  DO I=1,nombre_noeuds_DF-1
	...
	pas_espace_DF(I)=sqrt((x2-x1)**2+(y2-y1)**2)
        if (x1.gt.x2) then
		if (y1.lt.y2) then
		pas_espace_DF(I)=-pas_espace_DF(I)
		end if
		else
        	if (y1.gt.y2) then
		pas_espace_DF(I)=-pas_espace_DF(I)
...
!stop
  ...
  if (x1.gt.x2) then
	if (y1.lt.y2) then
	pas_espace_DF(nombre_noeuds_DF)=-pas_espace_DF(nombre_noeuds_DF)
	end if
	else
        if (y1.gt.y2) then
	pas_espace_DF(nombre_noeuds_DF)=-pas_espace_DF(nombre_noeuds_DF)
...
ENDDO
END SUBROUTINE creation_grille_diff_finies_signe
