...
allocate(B(NbCla,Nphi,Nphi))
!allocate(B2(NbCla,Nphi,Nphi))
B=0.D0
!B2=0.D0
NbCla=0
NbnoCla=0
...
!!! If there is a Neighbor
	...
	!! [u][v]
         DO J=1,Order+1
	       ...
	       B(Nbcla,Q,L)=B(Nbcla,Q,L)+res*sqrt(sum(V(JJ,:)**2))*2*(1-(cos(anglealpha1))**2)
            END DO
         END DO
      END IF
   END DO
