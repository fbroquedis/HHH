subroutine sub_rhselasto_acoustic_asymp(F,theta0)
  Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
...
F(I+PhiEdge(int_j,K),1) = F(I+PhiEdge(int_j,K),1)&
     &-d*sum(Phi(K,:)*Valsourcebord(:)*wGL1D)*norm(int_j,1)*(0D0,1D0)*omega
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! Calcul de  -i*omega*p_incident*n_y
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
F(I+Nphi+PhiEdge(int_j,K),1) = F(I+Nphi+PhiEdge(int_j,K),1)&
     &-d*sum(Phi(K,:)*Valsourcebord(:)*wGL1D)*norm(int_j,2)*(0D0,1D0)*omega
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
F(I+PhiEdge(int_j,K),1) = F(I+PhiEdge(int_j,K),1)&
     &+d*res*norm(int_j,1)*dsqrt(mu(Ineigh)/rho(Ineigh))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! Calcul de  -c*grad p_incident.n*n_y
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
F(I+Nphi+PhiEdge(int_j,K),1) = F(I+Nphi+PhiEdge(int_j,K),1)&
     &+d*res*norm(int_j,2)*dsqrt(mu(Ineigh)/rho(Ineigh))
              end do
...
end subroutine sub_rhselasto_acoustic_asymp
