SUBROUTINE sub_defstiffmat_elasto_acoustic_uyp_courbe(R0)
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
if(helmholtz.eq.1) then
              A_uyp(Iloc,L,(Arete-1)*Nphi+Q)=A_uyp(Iloc,L,(Arete-1)*Nphi+Q)+res
else
              A_uyp(Iloc,L,(Arete-1)*Nphi+Q)=A_uyp(Iloc,L,(Arete-1)*Nphi+Q)+res
           end if
           END DO
...
DO I=1,Nsolflu
   A_uyp(I,1:Nphi,1:3*Nphi)=Matmul(Minv,A_uyp(I,1:Nphi,1:3*Nphi))/DFVec(Nflu+Nflusol+I)/rho(I+Nflu+Nflusol)
ENDDO
end if
END SUBROUTINE sub_defstiffmat_elasto_acoustic_uyp_courbe
