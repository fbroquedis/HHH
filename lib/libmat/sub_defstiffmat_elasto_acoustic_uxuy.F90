SUBROUTINE sub_defstiffmat_elasto_acoustic_uxuy
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Computation of int(sigma(u): grad(v))
     DO J=1,Nphi
        DO K=1,Nphi
           ...
           A_uxuy(Iloc,J,K)=res*DFVEC(I)*Cij(Iloc,3,1)
           ...
           A_uxuy(Iloc,J,K)=A_uxuy(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,2,1) 
           ...
           res=res+Jfinv(1,2)*Jfinv(2,2)*GradPhiIGradPhiJ(J,K,2,2)     
           A_uxuy(Iloc,J,K)=A_uxuy(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,3,3) 
           ...
           A_uxuy(Iloc,J,K)=A_uxuy(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,2,3)  
...
!! Computation of int(-{sigma(u)n}.[v] -{sigma(v)n}.[u])
           DO J=1,Nphi
              ...
              !------------- C31*dux/dx*vy*nx & C32*dvy/dy*ux*ny -------------!
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dx*phi_I,L/2*nx
              A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+Cij(Iloc,3,1)*res*sqrt(sum(V(Arete,:)**2))/2.D0
              !d(phi_I,L)/dy*phi_I,J/2*ny
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+Cij(Iloc,3,2)*res*sqrt(sum(V(Arete,:)**2))/2D0
              ...
              A_uxuy(Iloc,J,Arete*Nphi+L)=A_uxuy(Iloc,J,Arete*Nphi+L)-Cij(Iloc,3,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_Ineigh,L)/dy*phi_I,J/2*ny 
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,L,Arete*Nphi+J)=A_uxuy(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,3,2)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C33*dux/dy*vy*nx & C33*dvy/dx*ux*ny -------------!
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dy*phi_I,L/2*nx
              A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2.D0
              !d(phi_I,L)/dx*phi_I,J/2*ny
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0

              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dy*phi_Ineigh,L/2*nx 
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,J,Arete*Nphi+L)=A_uxuy(Iloc,J,Arete*Nphi+L)-Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_Ineigh,L)/dx*phi_I,J/2*ny 
              res=-sum((Jfinv_voisin(1,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,L,Arete*Nphi+J)=A_uxuy(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C21*dux/dx*vy*ny & C12*dvy/dy*ux*nx -------------!
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dx*phi_I,L/2*ny
              A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+Cij(Iloc,2,1)*res*sqrt(sum(V(Arete,:)**2))/2.D0
              !d(phi_I,L)/dy*phi_I,J/2*nx
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+Cij(Iloc,1,2)*res*sqrt(sum(V(Arete,:)**2))/2D0

              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dx*phi_Ineigh,L/2*ny 
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,J,Arete*Nphi+L)=A_uxuy(Iloc,J,Arete*Nphi+L)-Cij(Iloc,2,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_Ineigh,L)/dy*phi_I,J/2*nx 
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,2+Order-K)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,L,Arete*Nphi+J)=A_uxuy(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,1,2)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C23*dux/dy*vy*ny & C13*dvy/dx*ux*nx -------------!
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dy*phi_I,L/2*ny
              A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+Cij(Iloc,2,3)*res*sqrt(sum(V(Arete,:)**2))/2.D0
              !d(phi_I,L)/dx*phi_I,J/2*nx
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+Cij(Iloc,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0

              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dy*phi_Ineigh,L/2*ny 
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,J,Arete*Nphi+L)=A_uxuy(Iloc,J,Arete*Nphi+L)-Cij(Iloc,2,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_Ineigh,L)/dx*phi_I,J/2*nx 
              res=-sum((Jfinv_voisin(1,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,2+Order-K)
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxuy(Iloc,L,Arete*Nphi+J)=A_uxuy(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!grad(phi_I,J)*phi_I,L/2
             A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+res*sqrt(sum(V(Arete,:)**2))*mu(I)
!grad(phi_I,L)*phi_I,J/2
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+res*sqrt(sum(V(Arete,:)**2))*mu(I)
           END DO
        END DO
!!$!!alpha/hmin [u][v]
!!$        DO J=1,Order+1
...
!!$              Q=PhiEdge(Arete,J)
!!$!phi_I,Q*phi_I,L
!!$              A_uxuy(Iloc,Q,L)=A_uxuy(Iloc,Q,L)+res*sqrt(sum(V(Arete,:)**2))
!!$           END DO
!!$        END DO
     END IF
  END DO
...
DO I=1,Nsolflu+Nsol
   A_uxuy(I,1:Nphi,1:4*Nphi)=Matmul(Minv,A_uxuy(I,1:Nphi,1:4*Nphi))/DFVec(I+Nflu+Nflusol)/rho(I+Nflu+Nflusol)
ENDDO
end if
END SUBROUTINE sub_defstiffmat_elasto_acoustic_uxuy
