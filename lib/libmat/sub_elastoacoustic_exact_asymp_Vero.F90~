...
subroutine elastoacoustic_exact_asymp_OSRC(k,kp,ks,R0,R1,Nsomme,PhiEdge,epsilon)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!     matsystcoeff(3,2) = (conjg(Hankel0(i)))
     matsystcoeff(1,1) = 2.d0*Cij(1,3,3)/(R0**2)*(((i-1)**2+(i-1)-1/2.d0*ks**2*R0**2)*J0p(i)-& 
          & kp*R0*(2.d0*(i-1)/(kp*R0)*J0p(i)-J0p(i+1)))
     matsystcoeff(1,1) = matsystcoeff(1,1)*((0D0,1D0)*k+(i-1)**2/(2D0*(0D0,1D0)*k*R0**2*heps**2)) 
 ...
 matsystcoeff(1,2) =  matsystcoeff(1,2)*((0D0,1D0)*k+(i-1)**2/(2D0*(0D0,1D0)*k*R0**2*heps**2)) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     if (i==1) then
        ...
        rhssystcoeff(1) =rhssystcoeff(1)*((0D0,1D0)*k+(i-1)**2/(2D0*(0D0,1D0)*k*R0**2*heps**2)) 
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_exact_asymp_OSRC
