SUBROUTINE sub_defmatD_rampant_bis
  Use m_mat
  Use m_gen
  Use m_mesh
  Use m_condbord
      ...
      IF(Ineigh.eq.-3) then
           DO J=1,1+Order
               DO K=1,1+Order
                  ...
                  Dclabis(I,J,K)=Dclabis(I,J,K)+res/sqrt(sum(V(JJ,:)**2))/rho(Tri_glob)**2
                  L=PhiEdge(JJ,K)
                  Ebis(I,J,L)=Ebis(I,J,L)+res/sqrt(sum(V(JJ,:)**2))/rho(tri_glob)**2
		end do
	  end do
	write(6,*) '1er',Dclabis(1,1,1)
...
!! Searching Neighbor
     ...
     IF(Jneigh.gt.0) then
        ...
        hmin=min(h1,h2)
        cmax=max(1D0/(tri_bord(I)),1D0/(tri_bord(Jneigh)))
...
!! Computation of -{grad u}. [v]-{grad v}.[u]
 ...
 !!alpha/hmin [u][v]
    	...
    	write(6,*) '4eme',Dclabis(1,1,1),L1+toto
     L1=L1+1+Order
     L1E=L1E+Nphi
...
END SUBROUTINE sub_defmatD_rampant_bis
