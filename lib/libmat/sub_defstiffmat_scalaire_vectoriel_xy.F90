SUBROUTINE sub_defstiffmat_scalaire_vectoriel_xy
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Computation of int( div (u) div( v))
!! Dans ce cas, c'est juste dux/dx.dvx/dx
     DO J=1,Nphi
        DO K=1,Nphi
           ...
           A_uxuy(Iloc,J,K)=res*DFVEC(I)*mu(I)
        END DO
     END DO
     L1=Nphi
     DO Arete=1,3
...
!d(phi_I,J)/dx*phi_I,L/2*ny
              A_uxuy(Iloc,J,L)=A_uxuy(Iloc,J,L)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2.D0
!d(phi_I,L)/dy*phi_I,L/2*nx
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              A_uxuy(Iloc,L,J)=A_uxuy(Iloc,L,J)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uxuy(Iloc,J,Arete*Nphi+L)=A_uxuy(Iloc,J,Arete*Nphi+L)-mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
!d(phi_Ineigh,J)/dy*phi_I,L/2*nx 
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,2+Order-K)
!div(phi_Ineigh,J)*phi_I,L/2 
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uxuy(Iloc,L,Arete*Nphi+J)=A_uxuy(Iloc,L,Arete*Nphi+J)+mu(Ineigh)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!!$        DO J=1,Order+1
!!$           DO K=1,Order+1
!!$              res=PhiIPhiJ(J,K)*2D0*alpha*cmax/hmin!*norm(Arete,1)*norm(Arete,2)
!!$              L=PhiEdge(Arete,K)
!!$              Q=PhiEdge(Arete,J)
!!$!phi_I,Q*phi_I,L
...
!!$              L=PhiEdge(Arete_neigh,2+Order-K)
!!$              A_uxuy(Iloc,Q,Arete*Nphi+L)=A_uxuy(Iloc,Q,Arete*Nphi+L)-res*sqrt(sum(V(Arete,:)**2))
!!$           END DO
!!$        END DO
     L1=L1+Nphi
!!! Dirichlet 
...
END SUBROUTINE sub_defstiffmat_scalaire_vectoriel_xy
