SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yx
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Dans ce cas, c'est juste dux/dx.dvx/dx
     DO J=1,Nphi
        DO K=1,Nphi
           ...
           res=res+Jfinv(2,1)*Jfinv(1,2)*GradPhiIGradPhiJ(J,K,1,2)
           res=res+(Jfinv(2,2)*Jfinv(1,1))*GradPhiIGradPhiJ(J,K,2,1)
           res=res+Jfinv(1,2)*Jfinv(2,2)*GradPhiIGradPhiJ(J,K,2,2)
           A_uyux(Iloc,J,K)=res*DFVEC(I)*mu(I)
        END DO
     END DO
     L1=Nphi
     DO Arete=1,3
...
!d(phi_I,J)/dy*phi_I,L/2*nx
              A_uyux(Iloc,J,L)=A_uyux(Iloc,J,L)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2.D0
!d(phi_I,L)/dx*phi_I,J/2*ny
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              A_uyux(Iloc,L,J)=A_uyux(Iloc,L,J)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uyux(Iloc,J,Arete*Nphi+L)=A_uyux(Iloc,J,Arete*Nphi+L)-mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
!d(phi_Ineigh,J)/dx*phi_I,L/2*ny 
              res=-sum((Jfinv_voisin(1,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uyux(Iloc,L,Arete*Nphi+J)=A_uyux(Iloc,L,Arete*Nphi+J)+mu(Ineigh)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!!$              L=PhiEdge(Arete_neigh,2+Order-K)
!!$              A_uyux(Iloc,Q,Arete*Nphi+L)=A_uyux(Iloc,Q,Arete*Nphi+L)-res*sqrt(sum(V(Arete,:)**2))
!!$           END DO
!!$        END DO
     L1=L1+Nphi
!!! Dirichlet 
...
END SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yx
