SUBROUTINE sub_defstiffmat_elasto_acoustic_asymp_dir_uyux(epsilon)
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
  ...
  REAL*8 :: norm(3,2),h1,h2,hmin,epsilon
  complex*16 :: res
...
!! Computation of p*u_x
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*norm(Arete,1)*norm(Arete,2)*rho(Ineigh)
              res=res*epsilon*omega**2*(1D0-epsilon/2D0/0.01D0)
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete,K)
!phi_I,L*phi_I_neigh,Q
If(helmholtz.eq.1) then
              A_uyux(Iloc,L,Q)=A_uyux(Iloc,L,Q)-res*sqrt(sum(V(Arete,:)**2))
else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
END SUBROUTINE sub_defstiffmat_elasto_acoustic_asymp_dir_uyux
