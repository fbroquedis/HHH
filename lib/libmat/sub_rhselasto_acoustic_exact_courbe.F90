subroutine sub_rhselasto_acoustic_exact_grad_courbe(F,theta0,R0,R1)
  Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
  ...
  real*8 :: coorx1,coorx2,coory1,coory2,cmax,R0,R1,theta1,theta2
  integer :: Node(3),I,int_j,J,l,NsommeNum,K,NCALC,Ineigh,PhiEdge(3,1+Order),Nsommen,Isol,J1,J2,ll
...
end subroutine sub_rhselasto_acoustic_exact_grad_courbe

subroutine sub_rhselasto_acoustic_exact_rot_courbe(F,theta0,R0,R1)
  Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
  ...
  REAL*8 :: Phi(1+Order,NGL1D),coord_points(4),normal(2)
...
!!$            do ll=1,2
!!$               GradPhiIG(ll)=gradphiig(ll)+Phi(K,l)*GradValsourcebord(l,ll)*wGL1D(l)*dsqrt(d)
!!$            end do
!!$         end DO
!!$
!!$
!!$              F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1) +&
!!$                   &sum(GradPhiIG*norm(int_j,:))/rho(J)*1d15
!!$           enddo
!!$        end IF
!!$     enddo
!!$  enddo
...
!                 d=d+sum(pt_courbe_y*gradphi1D(:,L))**2
                ...
                normal(2)=-sum(pt_courbe_x*gradphi1D(:,L))
!                normal=normal/dsqrt(d)
                 res=res+Phi(K,l)*(SigmaValsourcebord(L,1)*normal(1)&
                      &+SigmaValsourcebord(L,3)*normal(2))*wGL1D(l)
              end DO
              F(I+PhiEdge(int_j,K),1) = F(I+PhiEdge(int_j,K),1)&
                   &+res
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!                 d=d+sum(pt_courbe_y*gradphi1D(:,L))**2
                ...
                normal(2)=-sum(pt_courbe_x*gradphi1D(:,L))
!                normal=normal/dsqrt(d)
                 res=res+Phi(K,l)*(SigmaValsourcebord(L,3)*normal(1)&
                      &+SigmaValsourcebord(L,2)*normal(2))*wGL1D(l)
              end DO
              F(I+Nphi+PhiEdge(int_j,K),1) = F(I+Nphi+PhiEdge(int_j,K),1)&
                   &+res
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!write(6,*) F(I+PhiEdge(int_j,K),1),F(I+Nphi+PhiEdge(int_j,K),1)
  ...
  deallocate(GradValsourcebord)
  deallocate(Valsourcebord)
end subroutine sub_rhselasto_acoustic_exact_rot_courbe
