...
subroutine elastoacoustic_analytic_courbe(k,kp,ks,R0,R1,Nsomme,PhiEdge)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        DO KK=1,Order-1
           ...
           coor_y=Coor(Tri(J,1),2)+KK*V12(2)/Order
              ...
              coor_x=R0*dcos(theta1+kk*dtheta/real(order,8))
              coor_y=R0*dsin(theta1+kk*dtheta/real(order,8))
           end if
           r=dsqrt(coor_x*coor_x+coor_y*coor_y)
           call RJBESL(k*r,0.D0,Nsomme3+1,Jr,NCALC)	
           call RYBESL(k*r,0.D0,Nsomme3+1,Yr,NCALC)
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_analytic_courbe
