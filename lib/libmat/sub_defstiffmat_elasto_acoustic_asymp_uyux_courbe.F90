SUBROUTINE sub_defstiffmat_elasto_acoustic_asymp_uyux_courbe(R0)
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
  ...
  REAL*8 :: norm(3,2),h1,h2,hmin,R0,theta1,theta2,d,pi,pt_courbe_x(Order+1),pt_courbe_y(Order+1),dtheta
  complex*16 :: res
pi=2.d0*dasin(1.d0)
...
!! Computation of p*u_x
        DO J=1,Order+1
           DO K=1,Order+1
              ...
              ENDDO
              res=res*norm(Arete,1)*norm(Arete,2)*dsqrt(mu(Ineigh)*rho(Ineigh))
              res=res*(0D0,1D0)*omega
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete,K)
!phi_I,L*phi_I_neigh,Q
If(helmholtz.eq.1) then
              A_uyux(Iloc,L,Q)=A_uyux(Iloc,L,Q)-res
else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
END SUBROUTINE sub_defstiffmat_elasto_acoustic_asymp_uyux_courbe
