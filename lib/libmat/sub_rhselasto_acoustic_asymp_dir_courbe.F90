subroutine sub_rhselasto_acoustic_asymp_dir_courbe(F,theta0,epsilon,R0)
  Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
...
real*8 :: coorx1,coorx2,coory1,coory2,cmax,epsilon,theta1,theta2,R0,normal(2)
     ...
     DO int_j=1,3
      ...
      IF ((Ineigh.gt.Nflu).and.(Ineigh.le.Nflu+Nflusol)) then
        ...
        hmin=min(h1,h2)
         ...
         DO K=2,order
            pt_courbe_x(k)= R0*cos(theta1+(k-1)*dtheta/real(order,8))
            pt_courbe_y(k)= R0*sin(theta1+(k-1)*dtheta/real(order,8))
         ENDDO
           do l=1,NGL1D
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              Valsourcebord(l) = exp(+cmplx(0,(omega/sqrt(mu(1)/rho(1))*(&
                   &sum(pt_courbe_x*phi(:,l))*cos(theta0)+&
                   &sum(pt_courbe_y*phi(:,l))*sin(theta0)))))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!$                 d=d+sum(pt_courbe_y*gradphi1D(:,L))**2
               ...
               res=res+Phi(K,l)*sum(GradValsourcebord(l,:)*normal)*wGL1D(l)*normal(1)/d
         end DO
F(I+PhiEdge(int_j,K),1) = F(I+PhiEdge(int_j,K),1)&
     &-res*epsilon*(1D0-epsilon/2D0/R0)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!$                 d=d+sum(pt_courbe_y*gradphi1D(:,L))**2
                ...
                normal(2)=-sum(pt_courbe_x*gradphi1D(:,L))
               d=sqrt(normal(1)**2+normal(2)**2)
               res=res+Phi(K,l)*sum(GradValsourcebord(l,:)*normal)*wGL1D(l)*normal(2)/d
         end DO
F(I+Nphi+PhiEdge(int_j,K),1) = F(I+Nphi+PhiEdge(int_j,K),1)&
     &-res*epsilon*(1D0-epsilon/2D0/R0)
              end do
...
end subroutine sub_rhselasto_acoustic_asymp_dir_courbe
