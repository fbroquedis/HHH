SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yy
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Dans ce cas, c'est juste dux/dx.dvx/dx
     DO J=1,Nphi
        DO K=1,Nphi
           ...
           A_uyuy(Iloc,J,K)=res*DFVEC(I)*mu(I)
        END DO
     END DO
     L1=Nphi
     DO Arete=1,3
...
!grad(phi_I,J)*phi_I,L/2
              A_uyuy(Iloc,J,L)=A_uyuy(Iloc,J,L)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2.D0
!grad(phi_I,L)*phi_I,J/2
              A_uyuy(Iloc,L,J)=A_uyuy(Iloc,L,J)+mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uyuy(Iloc,J,Arete*Nphi+L)=A_uyuy(Iloc,J,Arete*Nphi+L)-mu(I)*res*sqrt(sum(V(Arete,:)**2))/2D0
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
!grad(phi_Ineigh,J)*phi_I,L/2 
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A_uyuy(Iloc,L,Arete*Nphi+J)=A_uyuy(Iloc,L,Arete*Nphi+J)+mu(Ineigh)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!!alpha/hmin [u][v]
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*2D0*alpha*cmax/hmin!*norm(Arete,2)**2
              L=PhiEdge(Arete,K)
              Q=PhiEdge(Arete,J)
!phi_I,Q*phi_I,L
...
END SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yy
