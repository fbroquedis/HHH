...
subroutine elastoacoustic_exact_asymp_Vero_Complete(k,kp,ks,R0,R1,Nsomme,PhiEdge,epsilon)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
  ...
  complex*16 :: tmp,c,d,dd,alphab,betab,ur,utheta,pr,alphac,betac,deltac,beta,gamma
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
 gamma13=2.6789385347d0
     ...
     beta=1
     gamma=(6/R0)**(1/3D0)*gamma23/gamma13*(1/2D0-(0,1)*sqrt(3D0)/2D0)*k**(2/3D0) 
  DO i=1,Nsomme+1
alphac=gamma*beta-k**2+(1/(2D0*R0)-(0D0,1D0)*k)*(gamma+beta-1/R0)-(gamma+beta-1/R0)/R0+(i-1)**2/R0**2
betac=(gamma+beta-1/R0)*k**2-(gamma*beta-k**2)*(1/(2D0*R0)-(0D0,1D0)*k)
betac=betac+(i-1)**2/R0**2*((0D0,1D0)*k-1/(2D0*R0)-(gamma+beta-1/R0))
deltac=betac/alphac 
!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Matrice du système
!!!!!!!!!!!!!!!!!!!!!!!!!!
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_exact_asymp_Vero_Complete
