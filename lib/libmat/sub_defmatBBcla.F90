SUBROUTINE sub_defmatBBcla
 ...
 ! Use m_cla
  ...
  implicit none
  !REAL*8 :: anglealpha
  INTEGER :: I,INeigh,Node(3),J,K,L,L1,L2,Q,JJ,NbnoCla,INFO
...
!allocate(TriNoCla(NTri-NbCLA))
allocate(BBCla(NbCla,Nphi,Nphi))
      ...
      IF(Ineigh.eq.-3) then
         ...
         DO J=1,Order+1
            DO K=1,Order+1
               ...
               L=PhiEdge(JJ,K)
               Q=PhiEdge(JJ,J)
           !!  Bcla(NbCla,Q,L)=Bcla(NbCla,Q,L)+res*sqrt(sum(V(JJ,:)**2))*veloc(I)
	       BBcla(NbCla,Q,L)=BBcla(NbCla,Q,L)+res*sqrt(sum(V(JJ,:)**2))*(1+cos(anglealpha1))*2
            END DO
         END DO
      END IF
   END DO
...
END DO
!allocate(BClainv(NbCla,Nphi,Nphi))
!  Bclainv=Bcla
!  CALL DPOTRF('U',Nphi,Bclainv,Nphi,INFO)
!  CALL DPOTRI('U',Nphi,BClainv,Nphi,INFO)
!Do JJ=1,NbCla
!  DO I=1,Nphi
!     DO J=I+1,Nphi
!        Bclainv(JJ,J,I)=Bclainv(JJ,I,J)
!     end DO
!  end DO
!end do
END SUBROUTINE sub_defmatBBcla
