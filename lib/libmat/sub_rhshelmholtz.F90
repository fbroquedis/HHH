subroutine sub_rhshelmholtz(F,theta0)
 Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
...
!!! Calcul du terme grad uinc*phiK
            ...
            If(Type_Media(J).eq.1) then
               ...
               F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1) -&
                    &d*sum(GradPhiIG*norm(int_j,:))/(2D0*rho(J))
            else
               F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1) +&
                    &d*sum(GradPhiIG*norm(int_j,:))/(2D0*rho(Ineigh))
           end If
     cmax=max(1D0/rho(J),1D0/rho(Ineigh))

If(Type_Media(J).eq.1) then
               F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1) -&
               &alpha*cmax/hmin*d*sum(Phi(K,:)*Valsourcebord(:)*wGL1D)
else
               F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1)+&
               &alpha*cmax/hmin*d*sum(Phi(K,:)*Valsourcebord(:)*wGL1D)
            end If
...
end subroutine sub_rhshelmholtz
