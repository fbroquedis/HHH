SUBROUTINE sub_defstiffmat_scalaire_vectoriel_py
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Computation of p*u_y
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*norm(Arete,2)
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete_neigh,2+Order-K)
!phi_I,L*phi_I_neigh,Q
If(helmholtz.eq.1) then
              A_puy(Iloc,L,(Arete-1)*Nphi+Q)=A_puy(Iloc,L,(Arete-1)*Nphi+Q)-omega**2*res*sqrt(sum(V(Arete,:)**2))
else
              A_puy(Iloc,L,(Arete-1)*Nphi+Q)=A_puy(Iloc,L,(Arete-1)*Nphi+Q)-res*sqrt(sum(V(Arete,:)**2))
end If
           END DO
        END DO
     END IF
...
END SUBROUTINE sub_defstiffmat_scalaire_vectoriel_py
