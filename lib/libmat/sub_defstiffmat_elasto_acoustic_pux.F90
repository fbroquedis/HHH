SUBROUTINE sub_defstiffmat_elasto_acoustic_pux
 Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
  INTEGER :: I,INeigh,Node(3),J,K,L,L1,Q,Arete,Arete_neigh,Iloc,test_arete
...
!! Computation of p*u_x
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*norm(Arete,1)
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete_neigh,2+Order-K)
!phi_I,L*phi_I_neigh,Q
If(helmholtz.eq.1) then
             A_pux(Iloc,L,(Arete-1)*Nphi+Q)=A_pux(Iloc,L,(Arete-1)*Nphi+Q)-omega**2*res*sqrt(sum(V(Arete,:)**2))
else
             A_pux(Iloc,L,(Arete-1)*Nphi+Q)=A_pux(Iloc,L,(Arete-1)*Nphi+Q)+res*sqrt(sum(V(Arete,:)**2))
endif
           END DO
        END DO
     END IF
...
END SUBROUTINE sub_defstiffmat_elasto_acoustic_pux
