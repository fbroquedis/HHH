subroutine sub_rhshelmholtzneu_scalaire_vectoriel(F,theta0)
 Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
         ...
         d=sqrt(sum(V(int_j,:)**2)) 
           do l=1,NGL1D
              ...
              GradValsourcebord(l,2) = +cmplx(0,omega/sqrt(mu(1)/rho(1)))*sin(theta0)&
                   ...
                   &((coorx2-coorx1)*PtGl1D(l)+coorx1)*cos(theta0)+&
                   &((coory2-coory1)*PtGl1D(l)+coory1)*sin(theta0)))))
!!$              Valsourcebord(l) = -exp(cmplx(0,(omega*(&
!!$                   &((coorx2-coorx1)*PtGl1D(l)+coorx1)*cos(theta0)+&
!!$                   &((coory2-coory1)*PtGl1D(l)+coory1)*sin(theta0)))))
!!$              GradValsourcebord(l,1) = -cmplx(0,omega)*cos(theta0)&
!!$                   &*exp(cmplx(0,(omega*(&
!!$                   &((coorx2-coorx1)*PtGl1D(l)+coorx1)*cos(theta0)+&
!!$                   &((coory2-coory1)*PtGl1D(l)+coory1)*sin(theta0)))))
!!$              GradValsourcebord(l,2) = -cmplx(0,omega)*sin(theta0)&
!!$                   &*exp(cmplx(0,(omega*(&
!!$                   &((coorx2-coorx1)*PtGl1D(l)+coorx1)*cos(theta0)+&
!!$                   &((coory2-coory1)*PtGl1D(l)+coory1)*sin(theta0)))))
!!$              Valsourcebord(l) =cos(omega*(((coory2-coory1)*PtGl1D(l)+coory1)))
!!$              GradValsourcebord(l,1) = 0.D0
!!$              GradValsourcebord(l,2) =-omega*sin(omega*(((coory2-coory1)*PtGl1D(l)+coory1)))
           enddo
	
!			Valsourcebord(:,1) = 1.d0
...
if((coory1.eq.4.D0).and.(coory2.eq.4.D0)) then
write(6,*)  '1',F((J-1)*Nphi+PhiEdge(int_j,K),1),J,int_j
endif
               F((J-1)*Nphi+PhiEdge(int_j,K),1) = F((J-1)*Nphi+PhiEdge(int_j,K),1) +&
&d*sum(GradPhiIG*norm(int_j,:))/rho(J)
if((coory1.eq.4.D0).and.(coory2.eq.4.D0)) then
write(6,*)  '2',F((J-1)*Nphi+PhiEdge(int_j,K),1),d*sum(GradPhiIG*norm(int_j,:))
endif
...
enddo
   ...
   enddo
enddo
write(6,*) maxval(realpart(F))
write(6,*) maxval(imagpart(F))
write(6,*) minval(realpart(F))
write(6,*) minval(imagpart(F))
  deallocate(Valsourcebord)
 ! deallocate(TriBordR0)
end subroutine sub_rhshelmholtzneu_scalaire_vectoriel
