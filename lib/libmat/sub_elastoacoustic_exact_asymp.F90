...
subroutine elastoacoustic_exact_asymp(k,kp,ks,R0,R1,Nsomme,PhiEdge,epsilon)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!     matsystcoeff(3,2) = (conjg(Hankel0(i)))
     matsystcoeff(1,1) = 2.d0*Cij(1,3,3)/(R0**2)*(((i-1)**2+(i-1)-1/2.d0*ks**2*R0**2)*J0p(i)-& 
          & kp*R0*(2.d0*(i-1)/(kp*R0)*J0p(i)-J0p(i+1)))
     matsystcoeff(1,1) = matsystcoeff(1,1)-(0D0,1D0)*omega*dsqrt(mu(1)*rho(1))/R0&
          &*((i-1)*J0p(i)-kp*R0*J0p(i+1))
     matsystcoeff(1,1) = matsystcoeff(1,1)+epsilon&
          &*((0D0,1D0)*omega*dsqrt(mu(1)/rho(1))*rho(1)/R0-mu(1)*(i-1)**2/R0**2)/R0&
          &*((i-1)*J0p(i)-kp*R0*J0p(i+1))
     matsystcoeff(1,2) = 2.d0*Cij(1,3,3)/(R0**2)*((i-1)*(-i*J0s(i)+ks*R0*(2.d0*(i-1)/(ks*R0)*J0s(i)-J0s(i+1))))
     matsystcoeff(1,2) =  matsystcoeff(1,2)-(0D0,1D0)*omega*dsqrt(mu(1)*rho(1))/R0*(i-1)*J0s(i)
     matsystcoeff(1,2) =  matsystcoeff(1,2)+epsilon*&
          &((0D0,1D0)*omega*dsqrt(mu(1)/rho(1))*rho(1)/R0-mu(1)*(i-1)**2/R0**2)/R0*(i-1)*J0s(i)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     if (i==1) then
!        rhssystcoeff(1) = (-k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
        rhssystcoeff(1) = (-(cmplx(0,1))**(i-1)*J0(i))-(0D0,1D0)/k&
          &*(k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
        rhssystcoeff(1) =rhssystcoeff(1)+epsilon*&
&((0D0,1D0)/k/R0-(i-1)**2/k**2/R0**2)&
          &*(k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
     else
!        rhssystcoeff(1) = (-2*k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
        rhssystcoeff(1) = (-2D0*(cmplx(0,1))**(i-1)*J0(i))-(0D0,1D0)/k&
          &*(2*k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
        rhssystcoeff(1) =rhssystcoeff(1)+2D0*epsilon*&
&((0D0,1D0)/k/R0-(i-1)**2/k**2/R0**2)&
          &*(k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
     endif
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_exact_asymp
