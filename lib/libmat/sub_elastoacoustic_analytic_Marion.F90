...
subroutine elastoacoustic_analytic_Marion(k,kp,ks,R0,R1,Nsomme,PhiEdge)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     matsystcoeff(2,1) = (k*((i-1)/(k*R1)*Hankel1(i)-Hankel1(i+1)) &
          &- ((0D0,1D0)*k-0.4*k**(1/3D0)/R0**(2D0/3D0))*Hankel1(i))
     matsystcoeff(2,2) =(k*((i-1)/(k*R1)&
          &*conjg(Hankel1(i))-conjg(Hankel1(i+1))) &
          &- ((0D0,1D0)*k-0.4*k**(1/3D0)/R0**(2D0/3D0))*conjg(Hankel1(i)))
...
write(6,*) 'Modes u', l
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_analytic_Marion
