SUBROUTINE sub_defstiffmat_elasto_acoustic_puy_courbe(R0)
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!                normal=normal/dsqrt(d)
                 res=res+Phi(J,L)*Phi(K,L)*wGL1D(L)*normal(2)
              ENDDO
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete_neigh,2+Order-K)
!phi_I,L*phi_I_neigh,Q
If(helmholtz.eq.1) then
              A_puy(Iloc,L,(Arete-1)*Nphi+Q)=A_puy(Iloc,L,(Arete-1)*Nphi+Q)-omega**2*res
else
              A_puy(Iloc,L,(Arete-1)*Nphi+Q)=A_puy(Iloc,L,(Arete-1)*Nphi+Q)-res
end If
           END DO
        END DO
     END IF
...
DO I=1,Nflusol
   A_puy(I,1:Nphi,1:3*Nphi)=Matmul(Minv,A_puy(I,1:Nphi,1:3*Nphi))/DFVec(Nflu+I)*mu(I+Nflu)
ENDDO
end if
END SUBROUTINE sub_defstiffmat_elasto_acoustic_puy_courbe
