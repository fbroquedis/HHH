...
subroutine elastoacoustic_analytic_OSRC(k,kp,ks,R0,R1,Nsomme,PhiEdge)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     matsystcoeff(2,1) = (k*((i-1)/(k*R1)*Hankel1(i)-Hankel1(i+1)) &
          &- ((0D0,1D0)*k+(i-1)**2/(2D0*(0D0,1D0)*k*R0**2*heps**2))*Hankel1(i))
     matsystcoeff(2,2) =(k*((i-1)/(k*R1)&
          &*conjg(Hankel1(i))-conjg(Hankel1(i+1))) &
          &- ((0D0,1D0)*k+(i-1)**2/(2D0*(0D0,1D0)*k*R0**2*heps**2))*conjg(Hankel1(i)))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     DO int_j=1,3
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 

           P_analytic((J-1)*Nphi+int_j)=P_analytic((J-1)*Nphi+int_j)+pr
           l=l+1
           pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
           res=abs(pr/P_analytic((J-1)*Nphi+int_j))
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
           ...
           pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
           res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(3,2)))
        enddo
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
           ...
           pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
           res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(1,2)))
        enddo
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
           ...
           pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
           res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(2,2)))
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        DO KK=1,Order-1
           ...
           do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
              ...
              pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                   &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
              res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(3,KK+1)))
           enddo
        ...
        DO KK=1,Order-1
           ...
           do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
              ...
              pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                   &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
              res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(1,KK+1)))
           enddo
        ...
        DO KK=1,Order-1
           ...
           do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
              ...
              pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                   &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
              res=abs(pr/P_analytic((J-1)*Nphi+PhiEdge(2,KK+1)))
           enddo
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme3+1)) 
           ...
           pr=(coeffA(l)*cmplx(Jr(l),Yr(l))+&
                &coeffB(l)*cmplx(Jr(l),-Yr(l)))*dcos((l-1)*theta)
           res=abs(pr/P_analytic((J-1)*Nphi+Nphi))
!!$write(6,*) l,res
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     DO int_j=1,3
        ...
        do while (res.gt.1D-60.and.(l.lt.Nsomme2+1)) 
           ...
           ur=(coeffC(l)*((l-1)/(kp*r)*Jrp(l)-Jrp(l+1))*kp+coeffD(l)*(l-1)/r*Jrs(l))*dcos((l-1)*theta)
           utheta=(-coeffC(l)*(l-1)/r*Jrp(l)-coeffD(l)*((l-1)/(ks*r)*Jrs(l)-Jrs(l+1))*ks)*dsin((l-1)*theta)  
           res=2*dsqrt(abs(ur)**2+abs(utheta)**2)/&
...
write(6,*) 'Modes u', l
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_analytic_OSRC
