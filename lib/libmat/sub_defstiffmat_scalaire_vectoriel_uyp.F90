SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yp
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
if(helmholtz.eq.1) then
              A_uyp(Iloc,L,(Arete-1)*Nphi+Q)=A_uyp(Iloc,L,(Arete-1)*Nphi+Q)+res*sqrt(sum(V(Arete,:)**2))
else
              A_uyp(Iloc,L,(Arete-1)*Nphi+Q)=A_uyp(Iloc,L,(Arete-1)*Nphi+Q)+res*sqrt(sum(V(Arete,:)**2))
           end if
           END DO
...
END SUBROUTINE sub_defstiffmat_scalaire_vectoriel_yp
