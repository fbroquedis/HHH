subroutine sub_rhselasto_acoustic_asymp_courbe(F,theta0,R0)
  Use m_condbord    
  Use m_mat
  Use m_gen
  Use m_mesh
...
real*8 :: coorx1,coorx2,coory1,coory2,cmax,theta1,theta2,R0
...
res=0D0
             DO L=1,NGL1D
                 d=sum(pt_courbe_x*gradphi1D(:,L))**2
                 d=d+sum(pt_courbe_y*gradphi1D(:,L))**2
               res=res+Phi(K,l)*sum(GradValsourcebord(l,:)*norm(int_j,:))*wGL1D(l)*dsqrt(d)
         end DO
F(I+PhiEdge(int_j,K),1) = F(I+PhiEdge(int_j,K),1)&
     &+res*norm(int_j,1)*dsqrt(mu(Ineigh)/rho(Ineigh))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! Calcul de  -c*grad p_incident.n*n_y
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
F(I+Nphi+PhiEdge(int_j,K),1) = F(I+Nphi+PhiEdge(int_j,K),1)&
     &+res*norm(int_j,2)*dsqrt(mu(Ineigh)/rho(Ineigh))
              end do
...
end subroutine sub_rhselasto_acoustic_asymp_courbe
