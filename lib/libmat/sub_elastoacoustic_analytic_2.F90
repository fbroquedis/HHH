...
subroutine elastoacoustic_analytic_2(k,kp,ks,R0,R1,Nsomme,PhiEdge)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
  ...
  real *8 :: WORK,coeff_idiot1,coeff_idiot2
...
!!$  write(6,*) 'Bessel', J0(1), Y0(1), J0(2), Y0(2), J0(5), Y0(5)
  ...
  condi=1!omega**2!1D0/(omega**2*rho(1)/k)
	condi1=1!(omega**2*rho(1)/k)!1e-13
!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! Calcul des coefficients 
!!!!!!!!!!!!!!!!!!!!!!!!!!
 open(20,FILE="matrice_reelle.dat")
 open(21,FILE="matrice_complexe.dat")
 open(22,FILE="rhs_reelle.dat")
 open(23,FILE="rhs_complexe.dat")
  DO i=1,Nsomme+1
write(6,*) i,J1(i)
!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Matrice du système
...
if(maxval(abs(matsystcoeff(:,3))).ne.0D0) then
coeff_idiot1=maxval(abs(matsystcoeff(:,1)))/maxval(abs(matsystcoeff(:,3)))
else 
coeff_idiot1=1
end if
if(maxval(abs(matsystcoeff(:,4))).ne.0D0) then
coeff_idiot2=maxval(abs(matsystcoeff(:,1)))/maxval(abs(matsystcoeff(:,4)))
else
coeff_idiot2=1
end if
matsystcoeff(:,3)=coeff_idiot1*matsystcoeff(:,3)
matsystcoeff(:,4)=coeff_idiot2*matsystcoeff(:,4)
!!!!!!!!!!!!!!!!!!!!!!!!!! 
...
do l=1,4
do j=1,4
write(20,*) real(matsystcoeff(l,j)) 
write(21,*) aimag(matsystcoeff(l,j)) 
end do
write(22,*) real(rhssystcoeff(l)) 
write(23,*) aimag(rhssystcoeff(l)) 
end do
if(maxval(abs(matsystcoeff(:,3))).ne.0D0) then

     call ZGESV( 4, 1, matsystcoeff, 4, IPIV, rhssystcoeff, 4, INFO )
rhssystcoeff(3)=rhssystcoeff(3)*coeff_idiot1
rhssystcoeff(4)=rhssystcoeff(4)*coeff_idiot2
...
res=10D0
!toto
  do while (res.gt.1D-10.and.(l.lt.Nsomme3+1)) 
res=0D0
           l=l+1
  Do J=1,Nflu+Nflusol
 ! Do J=Nflu,Nflu+Nflusol
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_analytic_2
