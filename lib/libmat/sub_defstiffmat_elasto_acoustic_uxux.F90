SUBROUTINE sub_defstiffmat_elasto_acoustic_uxux
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
...
!! Computation of int(sigma(u): grad(v))
     DO J=1,Nphi
        DO K=1,Nphi
           ...
           A_uxux(Iloc,J,K)=res*DFVEC(I)*Cij(Iloc,1,1)
           ...
           A_uxux(Iloc,J,K)=A_uxux(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,3,1) 
           ...
           res=res+Jfinv(1,2)*Jfinv(2,2)*GradPhiIGradPhiJ(J,K,2,2)     
           A_uxux(Iloc,J,K)=A_uxux(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,1,3) 
           ...
           A_uxux(Iloc,J,K)=A_uxux(Iloc,J,K)+res*DFVEC(I)*Cij(Iloc,3,3)      
...
!! Computation of int(-{sigma(u)n}.[v] -{sigma(v)n}.[u])
        DO J=1,Nphi
           DO K=1,Order+1
              ...
              A_uxux(Iloc,J,L)=A_uxux(Iloc,J,L)+Cij(Iloc,1,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_I,L)/dx*phi_I,J/2*nx
              A_uxux(Iloc,L,J)=A_uxux(Iloc,L,J)+Cij(Iloc,1,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              ...
              A_uxux(Iloc,J,Arete*Nphi+L)=A_uxux(Iloc,J,Arete*Nphi+L)-Cij(Iloc,1,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              res=-sum((Jfinv_voisin(1,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,2+Order-K)
              !d(phi_Ineigh,L)/dx*phi_I,J/2*nx 
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,L,Arete*Nphi+J)=A_uxux(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,1,1)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C13*dux/dy*vx*nx -------------!
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dy*phi_I,L/2*nx
              A_uxux(Iloc,J,L)=A_uxux(Iloc,J,L)+Cij(Iloc,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_I,L)/dy*phi_I,J/2*nx
              A_uxux(Iloc,L,J)=A_uxux(Iloc,L,J)+Cij(Iloc,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dy*phi_Ineigh,L/2*nx
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,J,Arete*Nphi+L)=A_uxux(Iloc,J,Arete*Nphi+L)-Cij(Iloc,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,1)
              L=PhiEdge(Arete,2+Order-K)
              !d(phi_Ineigh,L)/dy*phi_I,J/2*nx 
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,L,Arete*Nphi+J)=A_uxux(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,1,3)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C31*dux/dx*vx*ny -------------!
              res=-sum((Jfinv(1,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dx*phi_I,L/2*ny
              A_uxux(Iloc,J,L)=A_uxux(Iloc,J,L)+Cij(Iloc,3,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              !d(phi_I,L)/dx*phi_I,J/2*ny
              A_uxux(Iloc,L,J)=A_uxux(Iloc,L,J)+Cij(Iloc,3,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dx*phi_Ineigh,L/2*ny
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,J,Arete*Nphi+L)=A_uxux(Iloc,J,Arete*Nphi+L)-Cij(Iloc,3,1)*res*sqrt(sum(V(Arete,:)**2))/2D0
              res=-sum((Jfinv_voisin(1,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
              !d(phi_Ineigh,L)/dx*phi_I,J/2*ny 
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,L,Arete*Nphi+J)=A_uxux(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,3,1)*res*sqrt(sum(V(Arete,:)**2))/2D0

              !------------- C33*dux/dy*vx*ny -------------!
              res=-sum((Jfinv(2,:)*GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,K)
              !d(phi_I,J)/dy*phi_I,L/2*ny
              A_uxux(Iloc,J,L)=A_uxux(Iloc,J,L)+Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2.D0
              !d(phi_I,L)/dy*phi_I,J/2*ny
              A_uxux(Iloc,L,J)=A_uxux(Iloc,L,J)+Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              L=PhiEdge(Arete_neigh,2+Order-K)
              !d(phi_I,J)/dy*phi_Ineigh,L/2*ny
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,J,Arete*Nphi+L)=A_uxux(Iloc,J,Arete*Nphi+L)-Cij(Iloc,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
              res=-sum((Jfinv_voisin(2,:)*GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,2)
              L=PhiEdge(Arete,2+Order-K)
              !d(phi_Ineigh,L)/dy*phi_I,J/2*ny 
              !(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2+Order-K
              A_uxux(Iloc,L,Arete*Nphi+J)=A_uxux(Iloc,L,Arete*Nphi+J)+Cij(Ineigh-Nflu-Nflusol,3,3)*res*sqrt(sum(V(Arete,:)**2))/2D0
...
!!alpha/hmin [u][v]
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*alpha2*cmax/hmin!*norm(Arete,1)**2
              L=PhiEdge(Arete,K)
              Q=PhiEdge(Arete,J)
!phi_I,Q*phi_I,L
...
!!alpha/hmin [u][v]
        DO J=1,Order+1
           DO K=1,Order+1
              res=PhiIPhiJ(J,K)*2*alpha2/h1*mu(I)
              L=PhiEdge(Arete,K)
              Q=PhiEdge(Arete,J)
!phi_I,Q*phi_I,L
              A_uxux(Iloc,Q,L)=A_uxux(Iloc,Q,L)+res*sqrt(sum(V(Arete,:)**2))
...
END SUBROUTINE sub_defstiffmat_elasto_acoustic_uxux
