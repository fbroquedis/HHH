...
allocate(Bbis(Narete_du_bord,1+order,Nphi))
...
!!! If there is a Neighbor
      IF(Ineigh.eq.-3) then
         If (TestCla==0) then
           ...
           ! TriCla(NbCla)=I
         END If
         !write(6,*) Arete_du_bord(nbcla,1),Tri(I,phiedge(JJ,1))
         !! [u][v]
         DO J=1,Order+1
            DO K=1,Order+1
               res=PhiIPhiJ(J,K)
               L=PhiEdge(JJ,K)
               Bbis(nbCla,J,L)=Bbis(NbCla,J,L)+res*sqrt(sum(V(JJ,:)**2))
            END DO
         END DO
      END IF
END DO
...
END SUBROUTINE sub_defmatB_bis
