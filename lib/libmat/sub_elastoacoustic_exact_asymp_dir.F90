...
subroutine elastoacoustic_exact_asymp_dir(k,kp,ks,R0,R1,Nsomme,PhiEdge,epsilon)
  Use m_condbord    
  Use m_mat
  Use m_mesh
  Use m_gen
...
!     matsystcoeff(3,2) = (conjg(Hankel0(i)))
     matsystcoeff(1,1) = 2.d0*Cij(1,3,3)/(R0**2)*(((i-1)**2+(i-1)-1/2.d0*ks**2*R0**2)*J0p(i)-& 
          & kp*R0*(2.d0*(i-1)/(kp*R0)*J0p(i)-J0p(i+1)))
!!$     matsystcoeff(1,1) = matsystcoeff(1,1)-epsilon*omega**2*rho(1)&
!!$          &*(1D0-epsilon/2D0/R0)/R0*((i-1)*J0p(i)-kp*R0*J0p(i+1))
     matsystcoeff(1,1) = matsystcoeff(1,1)-epsilon*omega**2*rho(1)&
          &/R0*((i-1)*J0p(i)-kp*R0*J0p(i+1))
     matsystcoeff(1,2) = 2.d0*Cij(1,3,3)/(R0**2)*((i-1)*(-i*J0s(i)+ks*R0*(2.d0*(i-1)/(ks*R0)*J0s(i)-J0s(i+1))))
!!$     matsystcoeff(1,2) =  matsystcoeff(1,2)-epsilon*omega**2*rho(1)*(1D0-epsilon/2D0/R0)/R0*(i-1)*J0s(i)
     matsystcoeff(1,2) =  matsystcoeff(1,2)-epsilon*omega**2*rho(1)/R0*(i-1)*J0s(i)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     if (i==1) then
!        rhssystcoeff(1) = (-k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
!!$        rhssystcoeff(1) = (-(cmplx(0,1))**(i-1)*J0(i))-epsilon&
!!$          &*(1D0-epsilon/2D0/R0)*(k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1))) 
       rhssystcoeff(1) = (-(cmplx(0,1))**(i-1)*J0(i))-epsilon&
          &*(k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))

     else
!        rhssystcoeff(1) = (-2*k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
!!$        rhssystcoeff(1) = (-2*(cmplx(0,1))**(i-1)*J0(i))-epsilon&
!!$          &*(1D0-epsilon/2D0/R0)*(2*k*(cmplx(0,1))**(i-1)*((i-1)/(k*R0)*J0(i)-J0(i+1)))
        rhssystcoeff(1) = (-2*(cmplx(0,1))**(i-1)*J0(i))-epsilon&
...
write(6,*) 'Modes', res,l,Nsomme
  ...
  deallocate(coeffC)
  deallocate(coeffD)
end subroutine elastoacoustic_exact_asymp_dir
