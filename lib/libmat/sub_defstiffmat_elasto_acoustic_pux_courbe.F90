SUBROUTINE sub_defstiffmat_elasto_acoustic_pux_courbe(R0)
 Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
  ...
  REAL*8 :: norm(3,2),h1,h2,hmin,res,R0,theta1,theta2,d,pi,pt_courbe_x(Order+1),pt_courbe_y(Order+1),dtheta
  pi=2.d0*dasin(1.d0)
...
dtheta=dtheta+2*pi
   ...
   ENDDO
!!$d=R0*min(min(abs(theta1-theta2),abs(theta1-theta2-2*pi)),abs(theta1-theta2+2*pi))
...
!                normal=normal/dsqrt(d)
                 res=res+Phi(J,L)*Phi(K,L)*wGL1D(L)*normal(1)
              ENDDO
              L=PhiEdge(Arete,J)
              Q=PhiEdge(Arete_neigh,2+Order-K)
!phi_I,L*phi_I_neigh,Q

If(helmholtz.eq.1) then
             A_pux(Iloc,L,(Arete-1)*Nphi+Q)=A_pux(Iloc,L,(Arete-1)*Nphi+Q)-omega**2*res
else
             A_pux(Iloc,L,(Arete-1)*Nphi+Q)=A_pux(Iloc,L,(Arete-1)*Nphi+Q)-res
endif
           END DO
        END DO
     END IF
...
if (helmholtz.eq.0)then
DO I=1,Nflusol
   A_pux(I,1:Nphi,1:3*Nphi)=Matmul(Minv,A_pux(I,1:Nphi,1:3*Nphi))/DFVec(Nflu+I)*mu(I+Nflu)
ENDDO
end if
END SUBROUTINE sub_defstiffmat_elasto_acoustic_pux_courbe
