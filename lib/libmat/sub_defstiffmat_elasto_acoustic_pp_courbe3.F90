SUBROUTINE sub_defstiffmat_elasto_acoustic_pp_courbe3(R0,R1)
  Use m_mat
  Use m_gen
  Use m_mesh
  implicit none
  ...
  REAL*8 :: DF(2,2,NGL2D),DFVEC_bis(NGL2D),theta1,theta2,R0,R1,vectmp1(2),vectmp2(2),DFINV(2,2,NGL2D)
...
indice=3
END SELECT
IF((Neigh(I,J).le.Nflu+Nflusol).and.(Neigh(I,J).gt.0)) then
   DO K=2,order
indice=indice+1
!!!      x_k=x_1+(k-1)*(x_2-x_1)/order (x1 and x2 are the two vertices)
   ...
   if (dtheta.gt.pi) then
      write(6,*) '1', theta1,theta2
      dtheta=dtheta-2*pi
   elseif (dtheta.lt.-pi) then
      write(6,*) '2', theta1,theta2
      dtheta=dtheta+2*pi
   end if
if(Neigh(I,J).gt.Nflu+Nflusol) then
   ...
   DO K=2,order
      indice=indice+1
      pt_courbe_x(indice)= R1*dcos(theta1+(k-1)*dtheta/real(order,8))
      pt_courbe_y(indice)= R1*dsin(theta1+(k-1)*dtheta/real(order,8))
   ENDDO
ENDIF
END IF
ENDDO
...
indice=3
END SELECT
IF((Neigh(Ineigh,J).le.Nflu+Nflusol).and.(Neigh(Ineigh,J).gt.0)) then
   DO K=2,order
indice=indice+1
!!!      x_k=x_1+(k-1)*(x_2-x_1)/order (x1 and x2 are the two vertices)
      pt_courbe_x(indice)= pt_courbe_x(J1)+(k-1)&
...
END SUBROUTINE sub_defstiffmat_elasto_acoustic_pp_courbe3
