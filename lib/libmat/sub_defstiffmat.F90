...
!grad(phi_I,J)*phi_I,L/2
              A(J,L,I)=A(J,L,I)+res*sqrt(sum(V(Arete,:)**2))/(2.D0*rho(I))
!grad(phi_I,L)*phi_I,J/2
              A(L,J,I)=A(L,J,I)+res*sqrt(sum(V(Arete,:)**2))/(2.D0*rho(I))
...
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A(J,Arete*Nphi+L,I)=A(J,Arete*Nphi+L,I)-res*sqrt(sum(V(Arete,:)**2))/(2D0*rho(I))
              res=-sum((MATMUL(Jfinv_voisin,GradPhiIPhiJ(Arete_neigh,J,K,:)))*norm(Arete,:))
              L=PhiEdge(Arete,2+Order-K)
!grad(phi_Ineigh,J)*phi_I,L/2 
!(attention, sur l'element voisin on tourne en sens inverse, d'ou le 2-Order-K
              A(L,Arete*Nphi+J,I)=A(L,Arete*Nphi+J,I)+res*sqrt(sum(V(Arete,:)**2))/(2.D0*rho(Ineigh))
...
!!alpha/hmin [u][v]
        DO J=1,Order+1
           DO K=1,Order+1
              ...
              Q=PhiEdge(Arete,J)
!phi_I,Q*phi_I,L
              A(Q,L,I)=A(Q,L,I)+res*sqrt(sum(V(Arete,:)**2))
!phi_I,Q*phi_Ineigh,L
              L=PhiEdge(Arete_neigh,2+Order-K)
              A(Q,Arete*Nphi+L,I)=A(Q,Arete*Nphi+L,I)-res*sqrt(sum(V(Arete,:)**2))
           END DO
        END DO
     L1=L1+Nphi
!!! Dirichlet 
...
!!! Computation of -{grad u}. [v]-{grad v}.[u]
        DO J=1,Nphi
           DO K=1,Order+1
              ...
              res=-sum((MATMUL(Jfinv,GradPhiIPhiJ(Arete,J,K,:)))*norm(Arete,:))
!grad(phi_I,J)*phi_I,L/2
             A(J,L,I)=A(J,L,I)+res*sqrt(sum(V(Arete,:)**2))/rho(I)
!grad(phi_I,L)*phi_I,J/2
              A(L,J,I)=A(L,J,I)+res*sqrt(sum(V(Arete,:)**2))/rho(I)
           END DO
        END DO
!!alpha/hmin [u][v]
        DO J=1,Order+1
...
DO I=1,Ntri
   A(1:Nphi,1:4*Nphi,I)=Matmul(Minv,A(1:Nphi,1:4*Nphi,I))/DFVec(I)*mu(I)
ENDDO
end if
END SUBROUTINE sub_defstiffmat
