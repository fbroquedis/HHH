SUBROUTINE sub_writeunstruct_ux
  Use m_mesh
  Use m_output
  Use m_condbord
  Use m_gen
  ...
  real*8 ::Ecritbis,h5
  If (Order.eq.3) then
     Ncells=9*(Nsolflu+Nsol)
     Nnodes=Nphi*(Nsolflu+Nsol)
     else
        Ncells=(Nsolflu+Nsol)
        Nnodes=3*(Nsolflu+Nsol)
     Endif
if(Nbinst.eq.0) then
  open(20,FILE="FILM/Ux_real.vtu")
else
  open(20,FILE="FILM/Ux_imag.vtu")
end if
...
DO I=1,Nsolflu+Nsol
   DO J=1,3
      write(20,*) Coor(Tri(Nflusol+Nflu+I,J),1),Coor(Tri(Nflusol+Nflu+I,J),2), 0
   END DO
   IF (Order.eq.3) then
      Node = Tri(Nflusol+Nflu+I,:)
      ...
      DO K=1,Order-1
         x=Coor(Tri(Nflusol+Nflu+I,1),1)+K*V12(1)/Order
         y=Coor(Tri(Nflusol+Nflu+I,1),2)+K*V12(2)/Order
         write(20,*) x,y, 0
      ENDDO
      DO K=1,Order-1
         x=Coor(Tri(Nflusol+Nflu+I,2),1)+K*V23(1)/Order
         y=Coor(Tri(Nflusol+Nflu+I,2),2)+K*V23(2)/Order
         write(20,*) x,y, 0
      ENDDO
      DO K=1,Order-1
         x=Coor(Tri(Nflusol+Nflu+I,3),1)+K*V31(1)/Order
         y=Coor(Tri(Nflusol+Nflu+I,3),2)+K*V31(2)/Order
         write(20,*) x,y, 0
      ENDDO
      x=Coor(Tri(Nflusol+Nflu+I,1),1)+V12(1)/3.-V31(1)/3.
      y=Coor(Tri(Nflusol+Nflu+I,1),2)+V12(2)/3.-V31(2)/3.
         write(20,*) x,y, 0
   ENDIF
ENDDO
   write(20,*) '</DataArray>'
...
ENDIF
   write(20,*)'</DataArray>'
   write(20,*) '</Cells>'
   write(20,*) '<PointData Scalars="Ux">'
   write(20,*) '<DataArray type="Float64" Name="Ux" format="ascii">'
IF (Order.eq.3) then
   DO I=1,Nsolflu+Nsol
      DO J=1,Nphi
      write(20,*) Ux((I-1)*Nphi+J)
   END DO
   END DO
else
   DO I=1,Nsolflu+Nsol
...
END SUBROUTINE sub_writeunstruct_ux
