SUBROUTINE  sub_defgrid
  Use m_mesh 
  Use m_gen
  implicit none
  integer :: i,ixmin,ixmax,iymin,iymax,ix,iy,nb
  real*8 :: xmin,xmax,ymin,ymax,point(2),a(3,2),pointbis(2),det,point1(2)
...
!              write(6,*) 'bedew'
             ! write(6,*) 'point1',point
              call sub_transform(a,point,point1)
              !write(6,*) 'point2',point
              ...
              IF ((point1(1).ge.0.d0).and.(point1(2).ge.0.d0).and.(point1(1)+point1(2).le.1D0)) then
                 Triinterp(nb)=i
                 !write(6,*)'coucoutoto'
                 !write(6,*)'point3',point,I
                 !stop
                  SELECT CASE(ORDER)
                 CASE(1)
                    CALL Phi2DOrder1(point1(1),point1(2),1,ValPhigrid(nb,:))     
                 CASE(2)
                    CALL Phi2DOrder2(point1(1),point1(2),1,ValPhigrid(nb,:))     
                 CASE(3)
                    CALL Phi2DOrder3(point1(1),point1(2),1,ValPhigrid(nb,:))     
                 END SELECT
              END IF
           END DO
        ENDDO
