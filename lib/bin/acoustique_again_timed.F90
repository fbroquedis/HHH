...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
    ...
    real*8, allocatable, dimension(:) :: loop_times
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    allocate(loop_times(-1:ntfinal))
    loop_times = 0.0d0
    
    loop_times(-1) = omp_get_wtime()
...
!$OMP TASK DEFAULT(shared) PRIVATE(I) firstprivate(start_I)
            ...
            AU((I-1)*Nphi+1:I*Nphi)=-MATMUL(A(I,1:Nphi,1:Nphi),Us((I-1)*Nphi+1:I*Nphi,ptr_U))&
            &-MATMUL(A(I,1:Nphi,1*Nphi+1:(1+1)*Nphi),Us((Neigh(I,1)-1)*Nphi+1:Neigh(I,1)*Nphi,ptr_U))&
            &-MATMUL(A(I,1:Nphi,2*Nphi+1:(2+1)*Nphi),Us((Neigh(I,2)-1)*Nphi+1:Neigh(I,2)*Nphi,ptr_U))&
            &-MATMUL(A(I,1:Nphi,3*Nphi+1:(3+1)*Nphi),Us((Neigh(I,3)-1)*Nphi+1:Neigh(I,3)*Nphi,ptr_U))
...
!        END IF
        loop_times(N) = omp_get_wtime()
    END DO
!$OMP END SINGLE
!$OMP END PARALLEL
        
    timer_end = omp_get_wtime()
    print*, "   Time for loop = ", timer_end-loop_times(-1)
    print *, " "
    do i=-1,ntfinal-1
        loop_times(i) = loop_times(i+1) - loop_times(i)
    end do
    print *, loop_times
